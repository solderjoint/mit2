...
struct domain_t {
	uint32 next;
	uint32 freq[BUZZER_BUFLEN];
	uint16 num [BUZZER_BUFLEN];
} ;
 ...
 * **************************************************** */
void BuzzerDomainReset (void) { domain.next = 0; }
...
int32 BuzzerDomainSet (const uint16 num, const uint32 freq) {
	...
	uint32 ptr = (domain.next < BUZZER_BUFLEN)? domain.next : 0;
